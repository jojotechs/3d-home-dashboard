...
old_anchors=[dict(a.items()) for a in bpy.data.objects['city_lighting_rig']['lightAnchors']]
bpy.ops.wm.read_factory_settings(use_empty=True)
bpy.ops.import_scene.gltf(filepath=str(ROOT/'public/models/city-lighting-v1.glb'))
root=bpy.data.objects['city_lighting_rig'];anchors=root['lightAnchors']
expected=[a for a in old_anchors if not (-106<a['x']<-46 and 34<a['y']<78)]
assert [dict(a.items()) for a in anchors]==expected, 'Only the four old finance inserts may leave the shared rig'
assert root['assetVersion']=='0.8' and len(anchors)==82
...
report={'status':'passed','light_anchors':len(anchors),'mesh_objects':len(meshes),'emissive_materials':[m.name for m in emissive],'glb_bytes':(ROOT/'public/models/city-lighting-v1.glb').stat().st_size,'checks':['82 unique finite light placements survive GLB re-import','warm, blue and red emissive materials survive export','source metre units retained','companion model does not replace any district geometry']}
