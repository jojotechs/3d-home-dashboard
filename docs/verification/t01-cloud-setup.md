...
- 浏览器仅保留当前面板的余额内存与当前标签页的认证会话。认证存储键按标签页区分，避免 SDK 广播串用隔离会话。关闭财务面板会清空其金额内存；退出清除认证与金额，过期的异步响应不能重新写回。
...
2. Supabase Auth → Sign In / Providers：关闭 **Allow new users to sign up** 与匿名登录；保留 email/password。`supabase/config.toml` 仅表达本地配置，不会自动修改托管项目。[官方认证配置](https://supabase.com/docs/guides/auth/general-configuration)
...
5. Vercel 已由 `vercel.json` 指定 Vite、`npm run build` 和 `dist/client`。保留 `.openai/hosting.json`、Worker 与 Sites 打包脚本。[官方 Vite 部署](https://vercel.com/docs/frameworks/frontend/vite)
...
`HOUSEHOLD_ID` 使用固定 UUID，重试沿用同一 ID。运行 `npm run cloud:bootstrap`。脚本先验证公众注册已关闭，只在维护者环境使用管理 key；创建 Auth 身份后调用事务初始化家庭与管理员成员，不创建财务历史。账号创建和数据库事务属于两个服务边界：若后半段失败，保留输出的用户 ID 写入 `ADMIN_USER_ID` 后重试；不自动删除身份、不把首次网页访问者升级为管理员。
...
5. 断网保存不宣称成功、保留输入；恢复连接重试，历史只增加一次。读取最新版本后再编辑，不静默覆盖冲突。
