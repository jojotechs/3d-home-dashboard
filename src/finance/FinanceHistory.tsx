...
export function FinanceHistory({history, onRefresh, refreshing}: {history: FinanceHistoryEntry[]; onRefresh: () => void; refreshing: boolean}) {
  ...
  return <section className="finance-history" aria-label="财务历史与趋势">
    <div className="finance-list-heading"><h2>历史与趋势</h2><button className="text-button" disabled={refreshing} onClick={onRefresh}>刷新历史</button></div>
    {!history.length ? <p className="finance-empty">还没有财务历史。第一次保存后，这里会留下当时的完整记录。</p> : <>
